    ...
    procedure = text("""
        CREATE OR REPLACE FUNCTION update_app_user_role() RETURNS TRIGGER
            ...
            BEGIN
                ...
                SELECT INTO current_status au_aur_id
                FROM app_user
                WHERE au_id = NEW.aua_impacted_au_id;

                SELECT INTO next_status auat_end_aur_id
                FROM app_user_action_type
                WHERE auat_id = NEW.aua_auat_id
                AND (auat_start_aur_id = current_status
                    OR auat_start_aur_id IS NULL);

                UPDATE app_user
                SET au_aur_id = next_status
